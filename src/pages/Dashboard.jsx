...
import { Plus, Sparkles, TrendingUp, BookOpen, CheckCircle2, Clock } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link to="/settings" className="text-xs text-gray-400 hover:text-gray-600 transition-colors">
            Manage →
          </Link>
        </div>

        {catLoading ? (
          <SkeletonLoader count={6} variant="category" />
        ) : categories.length === 0 ? (
        ...
        {!catLoading && !statsLoading && stats?.total === 0 && (
          <div className="mt-8 bg-indigo-50 border border-indigo-100 rounded-lg p-6 text-center">
            ...
            <p className="text-xs text-indigo-600 mb-4">
              Add questions manually or generate targeted questions using AI.
            </p>
            <div className="flex justify-center gap-3">
              <Link
                to="/add-question"
                className="px-4 py-2 bg-white border border-indigo-200 text-indigo-700 rounded-md text-sm font-medium hover:bg-indigo-50 transition-colors"
              >
                Add Manually
              </Link>
              <Link
